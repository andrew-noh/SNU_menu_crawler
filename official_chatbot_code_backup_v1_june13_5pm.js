module.exports = function(bot) {
          ...
          if (current_time >= zone_start_time && current_time < zone_end_time) {
            ...
            for (var schedule = 0; schedule < currentschedule_min.length; schedule++) {
              if (cur_time_min > currentschedule_min[schedule] && cur_time_min < currentschedule_min[schedule + 1]) {
                ...
                context.session.shuttleTime = "현재 시각에 배차 간격은" + parsed_object[bus].time_window + "분이며, 서울대 입구역에서 출발시 약 " + mins_left + "분 남았어!!";
              };
            };
          }
  ...
  bot.setTask('SearchWeather', {
    action: function(dialog, context, callback) {
      ...
      var appId = '&appid=' + key;
      var weather_url = 'http://api.openweathermap.org/data/2.5/weather?lat=37.459814&lon=126.953166';
      var requestURL = weather_url + appId + "&lang=kr";

      request2({
        uri: requestURL
      }, function(err, res, body) {
        if (err) {
          context.session.error = err.message;
        } else {
